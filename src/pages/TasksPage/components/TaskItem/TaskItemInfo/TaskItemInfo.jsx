...
const TaskItemInfo = React.memo(({ isSubTask }) => {
  ...
  const responsible_peoples = isSubTask
    ? task?.members_subtask
        ?.filter((person) => person.member_type === "Ответственный" || person.member_type === "Ментор")
  ...
  const foundPerson = membersToMap?.find(person => person.member.surname.includes(user.surName));

  // console.log(
  //   foundPerson?.member_type === 'Ответственный' && task.allow_change_deadline === true && task.status !== 'Завершена' 
  //
  ...
  {task.status !== undefined && (
    (task.status.includes("Ждет") && responsible_peoples.includes(user.userId)) ||
    (task.status.includes("В процессе ") && responsible_peoples.includes(user.userId)) ? (
      <div style={{ display: "flex", gap: "15px" }}>

        <Button className={styles.btn2} onClick={() => {
          const status = task.status.includes("Ждет") ? "Начата" : "Приостановлена";
          updateStatus(status);
        }}>
          {task.status.includes("Ждет") ? "Начать" : "Приостановить"}
        </Button>
...
{task?.status === "Завершена" ? (
    ...
    <Button className={styles.btn1} onClick={() => setOpenModal(true)}>
      Досрочно завершить
    </Button>
  )}
              ...
              {task.attached_document === "" || task.attached_document === undefined || task.attached_document === null ? (
                ""
              ) : (
                <Button
              ...
              >
                {task.accounting === null || task.accounting === undefined ? null :<h3>Бюджет: {task?.accounting?.initial_budget}</h3>}
                <div style={{ display: "flex" }}>
                  <h3>Крайний срок:  {task.deadline_date === null ? 'Бессрочная' : task.deadline_date }</h3>
                  { foundPerson?.member_type === 'Ответственный' && task.allow_change_deadline === true && task.status !== 'Завершена' ? (
              ...
              <div className={styles.stage_bottom}>
                        ...
                        >
                          Offline
                              ...
                              <Popover
                                ...
                                >
                                  <ProfileCard userId={item.member.user} />
                                </Popover>
                              </>
                            )}
                          </PopupState>
                ...
                <div className={styles.task_section}>
                  <div className={styles.task_info__header3}>
                    ...
                    {task.status !== 'Завершена' && (responsible_peoples?.includes(user.userId) || task?.creator?.user === user.userId ) ?
                  <Button
                    className={styles.default_button}
