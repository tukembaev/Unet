...
import styles from "./../../../StatementPage/components/StatementTable/StatementTable.module.scss";
import {useNavigate} from "react-router-dom";
const CompletedTaskTable = ({completedTasks , searchTerm , SearchByTitle ,isSubtasks}) => {
  ...
  return (
    <table className={styles.table__wrapper}>
      ...
      {searchFilter.map((item) => {
          return (   
            <tr key={item.id} className={styles.table__row} onClick={()=> {isSubtasks ? navigate(`/subtask/${item.id}/`) : navigate(`/task/${item.id}/`)}}>
              <td className={styles.table__item}>
                <span className={styles.table__title}>{item.task_name} {item.subtask_name}</span>
              </td>
              <td className={styles.table__item}>
              {isSubtasks ?   <span className={styles.table__title}> {item.members_subtask[0].member.first_name} {item.members_subtask[0].member.surname} </span> : <span className={styles.table__title}> {item.members[0].member.first_name} {item.members[0].member.surname} </span> }  
              ...
              <td className={styles.table__item}>
                <span className={styles.table__title}>{item.edit_status_date}</span>
              </td>
              <td className={styles.table__item}>
                {item.is_critical === true ? <span className={styles.table__title}>Важная</span> : <span className={styles.table__title}>Стандартная</span>  }
              </td>
            </tr>
