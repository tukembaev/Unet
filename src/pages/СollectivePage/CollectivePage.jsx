...
import { SendInviteToTeam, getMyMembers, getMyTeam } from '../../service/CollectiveService'
...
team_id = createTeammatesObject(selectedEmployee);
 ...
 const myTeam = useSelector((state) => state.collective.members);
   ...
   const handleSubmit = async (event) => {
    ...
    else if (selectedEmployee.some(obj => myTeam?.some(confirmedObj => confirmedObj.value === obj?.user_id))) {
      setNotify({
        isOpen: true,
  ...
  <div className={styles.titile__wrapper}>
        ...
        <div style={{display:'flex' , flexWrap:'wrap' , gap:'25px'}}          >
            {myTeam === [] ? <h1 style={{color:'white'}}>У вас пустой состав, добавьте пользователей для совместной работы</h1> :
            <>
               {myTeam?.map((item, index) => {
           
           return (
            <div            {...bindTrigger(popupState)}>
            <div className={styles.user_card} onClick={()=>setIdClick(item.value)}>
            <img src={item.photo} style={{borderRadius:'50%' , width:'54px', height:'54px', objectFit:'cover' , marginTop:'7px' }} alt="" />
            <h4 style={{color:'white', paddingTop:'20px'}}>{item.label}</h4>
            </div> 
            </div>
      ...
      <div style={{display:'flex', flexDirection:'column'}}>
          <EmployeeSelectAllUserId selectedEmployee = {setSelectedEmployee} isMulti = {true}/>
