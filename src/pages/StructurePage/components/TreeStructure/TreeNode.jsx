...
export default function TreeNode({ node , isBranches }) {
  ...
  return (
    ...
    <div className={styles.node} >
      ...
      <div className={`${styles['node_title']} arrow arrow-down`} onClick={handleClick}>
        {hasSubbranches ? (isOpen ? '▼ ' : '▶ ') : ' '}
        {hasSubbranches ? 'Управление - ' : 'Отдел - '} {node.title}</div><img className={styles.plus} src={add} alt="" onClick={() => setState({isPaneOpen: true })} /></div>
      {hasSubbranches && isOpen && (
        ...
        <div className={styles.node_subbranches}  >
          {node.subbranches.map((subbranch) => (
            <div className={styles.subbranch} key={subbranch.title} >
                <div style={{display:'flex' ,gap:'15px'}}>
              <div className={styles.subbranch_title}>Сектор - {subbranch.title}</div> <img className={styles.plus} src={add} alt="" onClick={() => setState({ isSubbranch: true , subbranch: subbranch, isPaneOpen: true })}/></div>
            </div>
          ))}
        </div>
      )}
