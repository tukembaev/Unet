...
const StageDocuments = ({data   ,setOpenModal ,status , role}) => {
  return (
    <div className={styles.wrapper}>
        <div className={styles.header}>
            ...
            {status === 'Завершен' ? '':      <button
    
    style={{ display: "flex", background: "#0094FF" }}
  >
    <img src={pen} alt="" />{" "}

    <p style={{ paddingTop: "4px", paddingLeft: "4px" }} onClick={() => setOpenModal(true)}>
      Добавить{" "}
    </p>
  </button> }
        ...
        <div className={styles.body} style={{maxHeight:'350px' , overflowY:"auto"}}>
  ...
  return (
    <tr className={styles.table__row_left} key={item.file}>
      ...
      <td className={styles.table__item_left} style={{textAlign:'right'}}>
        <span className={styles.table__title_left}>
          <a href={item.file} download> <img className={styles.size} src={download} alt="" /></a>
        </span>
      </td>
    </tr>
  );
