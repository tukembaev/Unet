...
import { SendInviteToTeam, getTeamMember } from "../../../service/CollectiveService";
  ...
  let data;
   ...
   const members = useSelector((state) => state.collective.team_members);
     ...
     const handleSubmit = async (event) => {
      ...
      if(selectedEmployee === ''){
      ...
      else if (members?.members?.some(confirmedObj => confirmedObj.member === selectedEmployee)) {
        setNotify({
          isOpen: true,
        ...
        try {
          let response = await SendInviteToTeam([{team:members.id , user_id:selectedEmployee }]);
      ...
      <div className={styles.team__wrapper}>
        <div className={styles.team__header}>
          <div className={styles.title}>
            <span onClick={()=>navigate(-1)} style={{cursor:'pointer'}}> Команды</span> 
            <img
              src={right}
              alt=""
              style={{
        ...
        <div className={styles.member_add_header}>
          ...
          <button   onClick={() => setOpenModal(true)}>Добавить участника </button>
        </div>
...
{members?.members?.filter((item) => item.is_confirmed).map((item) => (
  <tr className={styles.table__row}>
    <td className={styles.table__item}>
      ...
      />
      <span className={styles.table__title} style={{ fontWeight: "700" }}>
        {item.label}
      </span>
    </td>
    <td className={styles.table__item}>
    ...
    <td className={styles.table__item}>
      <span className={styles.table__title}>{item.position}</span>
    </td>
    <td className={styles.table__item}>
      <span className={styles.table__title}>{item.number_phone}</span>
    </td>
    <td className={styles.table__item}>
  ...
  <tr className={styles.table__row}>
    ...
    <td className={styles.table__item}>
      ...
      <div style={{display:'flex', flexDirection:'column'}}>
          <EmployeeSelectAllUserId selectedEmployee = {setSelectedEmployee} isMulti = {false}/>
