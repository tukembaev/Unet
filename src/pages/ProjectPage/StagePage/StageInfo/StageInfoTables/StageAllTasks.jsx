    ...
    <Layout>
      <div className={styles.team__wrapper}>
        ...
        <div className={styles.member_add_header}>
          ...
          <div>
            {roleInProject === 'Таск-менеджер' || roleInProject === 'Процесс-менеджер' || members?.creator_name?.includes(user.surName) && members.status !== 'Завершен' && members.status.includes('Ждет выполнения с') === false ? <button onClick={() => setState({ isPaneOpen: true })}>
              Поставить задачу
            </button> : ''}
              ...
              {data && data?.tasks
                ...
                .map((item) => (
                  ...
                  >
                    ...
                    <td className={styles.table__item}>
                      <span className={styles.table__title}>{item.accounting?.initial_budget}</span>
                    </td>
                    <td className={styles.table__item}>
                      <span className={styles.table__title}>{item.accounting?.expenses}</span>
                    </td>
                    <td className={styles.table__item}>
                      <span className={styles.table__title}>{item.accounting?.current_budget}</span>
                    </td>
                    <td className={styles.table__item}>
                    ...
                    { roleInProject === 'Бухгалтер' &&  item?.accounting?.income_confirmed === false ? <td className={styles.table__item}>
                  <span className={styles.table__title}><img src={agree} className={styles.size} alt="" onClick={(e) => handleSubmit(e, item.id)}/></span>
                </td> : ''}
                  </tr>))}
            </tbody>
          </table>
      ...
      {width > 1000 ? (
        <SlidingPaneUtil size='50%' title='Новая задача' state={state} setState={setState}> <TaskStageForm budgetForTask={members?.accounting?.current_budget} stage_id = {members.id} setRender={setRender} setState={setState} request_type = {request_type}/> </SlidingPaneUtil>

      ) : (
        <SlidingPaneUtil size='100%' title='Новая задача' state={state} setState={setState}> <TaskStageForm budgetForTask={members?.accounting?.current_budget} stage_id = {members.id} setRender={setRender} setState={setState} request_type = {request_type}/> </SlidingPaneUtil>
      )}
