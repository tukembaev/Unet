...
import ChangePublick from "../../KpiContent/components/ChangePublick/ChangePublick";
  ...
  return (
    <Layout>
      <div className={styles.kpiEmployee}>
        <div className={styles.employee_points}>
          <div className={styles.close_Kpi}>
            <button onClick={() => navigate(-1)}>Закрыть</button>
            <p>{kpiEmployee?.employee_name}</p>
          </div>
          <h3>
            <span style={{ color: "#8B0000" }}>
              {kpiEmployee?.potential_points}
            </span>
            /<span style={{ color: "darkgreen" }}>{kpiEmployee?.score}</span>
          </h3>
        </div>
        <div className={styles.kpi__publick}>
          {kpiEmployee?.data?.length !== 0 ? (
            kpiEmployee?.data?.map((item) => (
