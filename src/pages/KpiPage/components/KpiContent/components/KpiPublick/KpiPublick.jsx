...
export default function KpiPublick({publiucks , setRender , setNotify , cantChange}) {
  // states
  ...
  return (
    <>
      {publiucks?.map((item) => {
        if (item?.creator_name === `${user?.surName} ${user?.firstName}`) {
          return (
            <div key={item?.id} className={styles.publick}>
              <div className={styles.publick__head}>
                ...
                <div style={{display: 'flex',  flexDirection: 'row-reverse', alignItems: 'center', gap: '10px'}} className={styles.publick__status}>
                  ...
                  {item?.status === "Отказано" ? (
                    <div className={styles.rejected_publick}>
                      <p>{item?.status}</p>
                      <>
                      {cantChange? (null) : (<button onClick={() => handleDelete(item.id)}>
                        Удалить
                      </button>)}
              ...
              <div className={styles.publick__body}>
                <div className={styles.publick__body__head}>
                  ...
                  >
                    ...
                    <p>Дата создания {item?.published}</p>
                    <p>От лица : {item?.position}</p>
                    {item?.files !== 0 ? (
  <div className={styles.publick_files}>
    {item?.files?.map((item) => {
      // Extract the filename from the URL
      const filename = decodeURIComponent(item?.file).substring(item?.file.lastIndexOf('/') + 1);

      return (
        <a href={item?.file} download target="_blank" key={item?.id}>
