...
import stylese from '../PersonalCard/components/BodyCheckInfo/BodyCheckInfo.module.scss'
      ...
      <div className={styles.personal_wrapper2}>
        <div className={styles.header}>
          ...
          <div className={styles.header_body}>
            ...
            <button className={styles.btn1} style={{ marginLeft: '25px' }} onClick={handlePrint}>Распечатать</button>
          </div>
        </div>

        <button onClick={() => navigate('/europass')} style={{ width: '100px', marginRight: '30px' }} className={styles.btn1}>Назад</button>


        <button onClick={() => {
          setLogo(true)
        }} className={styles.btn1}>Выберите Логотип</button>

        {logo ? (
          <div style={{ marginTop: '20px', width: '330px', display: 'flex', justifyContent: 'space-between' }}>
            <img onClick={() => {
              setPolit(false)
              setEuro(true)
              setUnet(false)
            }} style={{ width: '100px', cursor: 'pointer' }} src={eurologo} alt="" />
            <img onClick={() => {
              setPolit(true)
              setEuro(false)
              setUnet(false)
            }} style={{ width: '50px', cursor: 'pointer' }} src={politLogo} alt="" />
            <img onClick={() => {
              setPolit(false)
              setEuro(false)
              setUnet(true)
            }} style={{ width: '80px', cursor: 'pointer' }} src={unetLogo} alt="" />
          </div>
        ) : (null)}
        <div className={styles.body2} style={{ margin: '0 auto', border: 'none', zIndex: '5', maxHeight: '3500px', display: 'flex', flexDirection: 'column' ,overflowX: 'auto'}} ref={componentRef}>

          <div style={{ paddingTop: '45px', justifyContent: 'center', display: 'flex', gap: '200px' }}>
            <div style={{ width: '150px' }}>
              {polit ? (
                <div style={{ width: '150px', margin: 'auto', paddingInline: '50px', paddingTop: '30px' }}>

                  <img style={{ zIndex: '2', objectFit: 'contain' }} src={politLogo} alt="" />
                </div>
              ) : (null)}

              {euro ? (<img style={{ width: '150px', height: '190px', zIndex: '2', objectFit: 'contain' }} src={eurologo} alt="" />) : (null)}

              {unet ? (<img style={{ width: '150px', height: '190px', zIndex: '2', objectFit: 'contain' }} src={unetLogo} alt="" />) : (null)}
            </div>
            <div>
              <img src={user.image} alt="" style={{ width: '150px', height: '200px', zIndex: '2', objectFit: 'cover', border: '1px solid #eee' }} />
            </div>
          </div>
            ...
            <div className={styles.body_left} style={{ zIndex: '5', gap: '0' }}>
              {euro ? (
                 <div className={styles.body_list}>
                    <h2 className={styles.body_list_left}  >Europass  Curriculum Vitae</h2>
                  </div>
              ) : (null)}
              {polit ? (
                 <div className={styles.body_list}>
                 <h2 className={styles.body_list_left}  >KSTU Curriculum Vitae</h2>
               </div>
              
              ) : (null)}
              {unet ? (
                <div className={styles.body_list}>
                <h2 className={styles.body_list_left}  >UNET  Curriculum Vitae</h2>
              </div>
             
              ) : (null)}
              {euro || polit || unet ? (null) : (<h2>ㅤㅤㅤ</h2>)}
              <div className={styles.body_list}>
                    <h3 style={{fontSize: '22px'}} className={styles.body_list_left}  >Личная информация</h3>
              ...
              <div className={styles.body_info}>
                ...
                <p style={{fontSize:'16px'}}>{check.first_name} {check.surname}</p>
              </div>

              <div className={styles.body_info}>
              {info?.residence_place?.street ? (<h4 className={styles.body_list_left}  >Адрес</h4>) : (null)}
                {info?.residence_place?.street}
              </div>

              <div className={styles.body_info}>
                {user_changes?.numberPhone ? (<h4 className={styles.body_list_left}  >Мобильный</h4>) : (null)}
                {user_changes?.numberPhone ? (<p >{user?.numberPhone}</p>) : (null)}

              </div>

              <div className={styles.body_info}>
                {user_changes?.email ? (<h4 className={styles.body_list_left} >Почта</h4>) : (null)}
              ...
              <div className={styles.body_info}>
                {user_changes?.data_of_birth ? (<h4 className={styles.body_list_left}  >Дата рождения</h4>) : (null)}
                {user_changes?.data_of_birth ? (<p >{user?.data_of_birth}</p>) : (null)}
              </div>
              ...
              {!check?.ranks || check?.ranks.length === 0  ? ( null
                ...
                <div className={styles.body_list}>
                    <h2 className={styles.body_list_left}  >Дипломатический ранг</h2>
                  </div>
                <div className={styles.body_list}>
              ...
              <div className={styles.body_info}>
                {check?.military_registration ? (
                  <div>
                    ...
                    <div className={styles.body_list}>
                      <h4 className={styles.body_list_left} >Годность к военной службе</h4>
                      {check?.military_registration.is_fit ? (<h4 style={{}}>Годен</h4>) : (<h4 style={{}}>Негоден</h4>)}
                    </div>
                    <div className={styles.body_list}>
                      <h4 className={styles.body_list_left} >Военкомат по месту жительства</h4>
              ...
              <div className={styles.body_info}>
                {check?.employment  ? (<div>
                  <div className={styles.body_list}>
                  <h2  className={styles.body_list_left}  >Трудовая деятельность</h2>
                  ...
                  <div className={styles.body_list}>
                    <h4 className={styles.body_list_left} >Частная структура</h4>
              ...
              <div className={styles.body_info}>
                {!check?.trainings || check?.trainings.length === 0  ? (null
                  ...
                  <div className={styles.body_list}>
                  <h2  className={styles.body_list_left}  >Повышение Квалификации</h2>
              ...
              <div className={styles.body_info}>
                {check?.employment === true && check?.employment?.experiences.length !== 0  ? (<div>
                  <div className={styles.body_list}>
                  <h2  className={styles.body_list_left}  >Выполняемая работа</h2>

                  </div>
                  
                  <div className={styles.body_list}>
                    <h4 className={styles.body_list_left} >Занимаемая должность</h4>
                  ...
                  <div className={styles.body_list}>
                    <h4 className={styles.body_list_left}>Наименование организации</h4>
                    <p>{info?.employment?.experiences[0].organization}</p>
