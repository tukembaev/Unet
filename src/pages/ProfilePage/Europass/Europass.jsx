...
import { setEuropass, setPersonalInfo, setUser_change } from '../../../store/slices/PersonalCard'
...
const getData = async () => {
 ...
 useEffect(() => {getData();} , [email,phone,citizen,address,birth_date,education,diplomatic,military,activity,trainQualif,profession])
    ...
    <div className={styles.personal_wrapper}>
      <div className={styles.header}>
        ...
        <div className={styles.header_body}>
            ...
            <button className={styles.btn1}  style={{}} >Русский </button>
            <button className={styles.btn1} onClick={() => navigate('/europassen') }>English</button>
            <button className={styles.btn1} onClick={() => navigate('/europassky') }>Кыргызча</button>
         </div>
          </div>
      </div>
    <button style={{width:'150px', marginLeft:'30px'}} onClick={() => navigate('/personalcard') } className={styles.btn1}>Назад</button>
      ...
      <h3>Отметьте поля которые хотите скрыть </h3>
        ...
        <div className={styles.list}>
          ...
          <h3>Личные данные</h3>
            <div className={styles.item}>
              <input onChange={(e) => setEmail(e.target.checked) } checked={email} type="checkbox" className={styles.radio} />
              <div>
              <h4>Почта</h4>
              <p>{user?.email}</p>
            </div>
            <div className={styles.item}>
              <input onChange={(e) => setPhone(e.target.checked) } checked={phone}  type="checkbox" className={styles.radio} />
              <div>
            ...
            <div className={styles.item}>
              <input onChange={(e) => setBirth_date(e.target.checked) } checked={birth_date} type="checkbox" className={styles.radio} />
              <div>
              <h4>Дата рождения</h4>
        ...
        <div className={styles.list}>
          ...
          <h3>Образование</h3>
            <div className={styles.item}>
              <input  onChange={(e) => setEducation(e.target.checked) } checked={education} type="checkbox" className={styles.radio} />
              <div>
              <h4>ВУЗ</h4>
      ...
      {info?.ranks?.length  === 0 ? (null) : (<div className={styles.list}>
          
          <h3>Дипломатический ранг или иные звания </h3>
            <div className={styles.item}>
              <input  onChange={(e) => setDiplomatic(e.target.checked) } checked={diplomatic} type="checkbox" className={styles.radio} />
              <div>
        ...
        <div className={styles.list}>
          ...
          <h3>Воинский учет </h3>
            <div className={styles.item}>
              <input  onChange={(e) => setMilitary(e.target.checked) } checked={military} type="checkbox" className={styles.radio} />
              <div>
              <h4>Годность к военной службе</h4>
              {info?.military_registration?.is_fit? (<p>Годен</p>) : (<p>Не Годен</p>)}
        ...
        <div className={styles.list}>
          ...
          <h3>Трудовая деятельность/Стаж работы </h3>
            <div className={styles.item}>
              <input  onChange={(e) => setActivity(e.target.checked) } checked={activity} type="checkbox" className={styles.radio} />
              <div>
              <h4>Специальность</h4>
        ...
        {info?.trainings?.length === 0 ? (null) : (<div className={styles.list}>
          
          <h3>Повышение Квалификации</h3>
            <div className={styles.item}>
            <input  onChange={(e) => setTrainQualif(e.target.checked) } checked={trainQualif} type="checkbox" className={styles.radio} />
        ...
        {info?.employment?.experiences.length === 0 ? (null) : (<div className={styles.list}>
          
          <h3>Выполняемая работа с начала трудовой деятельности</h3>
            <div className={styles.item}>
              <input  onChange={(e) => setProfession(e.target.checked) } checked={profession} type="checkbox" className={styles.radio} />
              <div>
            ...
            <div className={styles.item}>
              ...
              <h4>Штат</h4>
              <p>{info?.employment?.experiences?.slice(-1)[0].organization}</p>
              </div>
        ...
        <button className={styles.btn1} style={{marginLeft:'25px', height: '50px'}} onClick={() => navigate('/finaleuro')}>Формировать резюме</button>
      </div>
