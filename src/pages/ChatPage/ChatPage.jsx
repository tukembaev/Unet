...
import { createChat, getChatData, getEmployee_two } from '../../service/ChatService'
import { setChats, setEmployees, setMessageChat } from '../../store/slices/ChatSlice'
...
const ChatPage = ({ all_chats, setMessageCount, setRender, setId, setLoading, loading }) => {
	const user = userInfo();
	...
	let check = all_chats.messagechat.filter((item) => item.members[1].user_id === employee_two_post)
	const createNewChat = async () => {
		try {
			...
			let response = await createChat([{ employee: employee_two_post }, { employee: employee_myself }]);
	...
	const filteredArray = filterMembersBySurname(all_chats.messagechat, user.surName);
	...
	return (
		<Layout>
			...
			<div id="frame">
					...
					<div id="contacts">
						<ul>
							<li class="add-contact-modify" >
								<div class="wrap">
									{width > 736 ? <h1>Чаты <span title="Добавить чат" class="plus-icon" onClick={handleOpen}>+</span></h1> : <span class="plus-icon" onClick={handleOpen}>+</span>}
							...
							{filteredArray?.map((item) => {
								return (
									<li class="contact" onClick={() => handleClick({ id: item.id, name: item.employee_name, image: item.image })} >
										<div class="wrap">
											...
											<div class="image-container">
												<img src={item.image} alt="" />
												{item?.isOnline? (<div class="gray-circle"></div>) : null}
											
											</div>
											<div class="meta">
												<p class="name">{item.employee_name}</p>
												<div class='space_between'>
													<p class="preview">{item.last_message === null ? 'Напишите ваше первое сообщение' : item.last_message}</p>
				...
				<ChatContent name={name} image={image} userId={userId} id={selectedChatTitle} setId={setSelectedChatTitle} item={all_chats.messageId} setRender={setRender} loading={loading} />
				...
				>
					<div>
						<EmployeeSelectUserId selectedEmployee={setSelectedEmployee} setSelectedEmployeeLabel={setSelectedEmployeeLabel} isMulti={false} />
