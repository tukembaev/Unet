...
//   return $api.post("/conversion/raportsforpost/",{addressee,type, type_doc,text,file });
// }
...
export async function createRepair(employee,type,podtypezayavki,text , file) {
  return $api.post("/conversion/remont/",{employee,type,podtypezayavki,text , file});
}

export async function createOrgTech(employee,type,podtypezayavki,text , file) {
  return $api.post("/conversion/orgtech/",{employee,type,podtypezayavki,text , file});
}

export async function createRequirement({company_organization, views_operations,date_today, type,warehouse,department,requestedmen,uniq_codes, purchaselistproducts}) {
  return $apiJSON.post("conversion/trebovaniya/ ",{company_organization, views_operations,date_today , type,warehouse,department,requestedmen,uniq_codes,purchaselistproducts});
}

export async function createWriteOff({employee,type,podtypezayavki,text,spisanie}) {
  return $apiJSON.post("/conversion/spisanie/",{employee,type,podtypezayavki,text,spisanie});
}
