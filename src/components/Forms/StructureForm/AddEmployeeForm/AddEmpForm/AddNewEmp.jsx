...
import { GetPositionOfStructure, createEmpToStructure } from '../../../../../service/StructureService';
import { setNewEmp, setPositionOfStructureById } from '../../../../../store/slices/StructureSlice';
const AddNewEmp = ({structure_name , id , render,  setRender}) => {
    const [id2, setId2] = useState('');
    const [id3, setId3] = useState('');
    const [type_doc, setType_doc] = useState('');
    const request_type = [{id: 0, label: "Основной"}, {id: 1, label: "Совмещение"}];
    const request_type2 = [{id: 0, label: "0.25"}, {id: 1, label: "0.5"}, {id: 2, label: "1"}, {id:3, label: "1.5"}];
    ...
    const handleSubmit = async (event) => {
      ...
      if(employee === ''){
        ...
        try {
         ...
         let state = type_doc;
         let stavka = bet;
         let response = await createEmpToStructure(structure_name, id , {mov_employee_id , post, state ,stavka });
          dispatch(
            setNewEmp({ 
  ...
  return (
    ...
    <div className={styles.flex_col}>
       ...
       <div style={{display:'flex' , flexDirection:'column' , maxWidth:'400px'}}>
          ...
          <PositionSelect dataPosition={positions} setDataPosition = {setStateName}/>
       </div>
       <div style={{display:'flex' , flexDirection:'column' , width:'180px'}}>
       <h4>Ставка</h4>
       <Dropdown setId={setId2} setType={setBet} title = {'Выбрать ставку'} data = {request_type2 ?? []}/>
      </div>
      
      <div style={{display:'flex' , flexDirection:'column' , width:'180px'}}>
       <h4>Штат</h4>
       <Dropdown setId={setId3} setType={setType_doc} title = {'Выбрать штат'} data = {request_type ?? []}/>
      </div>
      <Button className = {styles.add_pos} onClick={handleSubmit}>Добавить </Button>
     </div>
