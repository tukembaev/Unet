    ...
    if(positionName === ''){
      ...
      try {
       ...
       let wage = parseInt(bet);
        let response = await createPositionToStructure(structure_name, id , {title, state ,wage });
        dispatch(
          setNewPosition({ 
            title:positionName,
            state:type_doc,
  ...
  return (
    <div className={styles.add_emp_position_wrapper}>
      ...
      <div style={{display:'flex' , flexDirection:'column' , width:'180px'}}>
       <h4>Штат</h4>
       <Dropdown setId={setId2} setType={setType_doc} title = {'Выбрать штат'} data = {request_type ?? []}/>
      </div>
