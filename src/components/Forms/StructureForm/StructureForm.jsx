...
import { setSend } from "../../../store/slices/StructureSlice";
...
const StructureForm = () => {
  ...
  let request_type = [
    { id: 0, label: "Управление" },
 
  ];  
  let request_type_branch = [
    { id: 0, label: "Отдел" },
          ...
          <div className={styles.structure_form}>
            ...
            >
              ...
              <div style={{display:'flex' , gap:'15px'}}>
                {dynamicInput.length !== 0 ? <button onClick={() => handleClear(true)} className={styles.btn_clear}>
  Очистить Управления
</button> :  ''  }
         
{dynamicInputBranch.length !== 0 ? <button onClick={() => handleClear(false)} className={styles.btn_clear}>
  Очистить Отделы
</button> :  ''  }
...
{dynamicInputBranch.length === 0 ? (
            ...
            <button className={styles.btn_save_structure} onClick={handleSubmit}>
