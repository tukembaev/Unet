...
const DynamicFields = ({ component, onValueChange , handleAddComponent , type , department ,request_type , title  }) => {
    ...
    const handleChange = (event) => {
      ...
      const handleAddComponent2 = () => {
        setDynamicInput2(prevComponents => [...prevComponents, { id: Date.now(), title: '' }]);
    };
  ...
  return (
    ...
    <div className={styles.card_field}>
        ...
        <input className={styles.title} placeholder={`${type} - ${title}`} type="text" value={component.value} onChange={handleChange} />
        <div style={{  display:'flex',  width: "24px",
           paddingBottom:'11px', marginLeft:'10px'}}>
           <h1  src={add} onClick={handleAddComponent}> + </h1>
        </div>
        </div>
        {dynamicInput2.length === 0  ?      <div style={{width:'250px' , paddingTop:'15px'}}>
          <Dropdown setId={setId} setType={setType2} handleAddComponent = {handleAddComponent2} title = {'Добавить поле'} data = {request_type ?? []}/>
          </div> : ''}
          <div style={{display:'flex', gap:'20px' , paddingTop:'25px', borderBottom:'1px solid black'}}>
